...
def add_activity():
    ...
    button = Button(filewin, text="add",
                    command=lambda: add_new_activity(filewin))
    button.grid(row=i, column=1)
        ...
        if CheckVar[index].get() == 1:
            ...
            if thing != '':
                if len(thing.split(';')) == 1:
                    Ann.done_task(Ann.today[index], float(
                        Ent[index].get()), day0=CURRENTDAY)
                else:
                    things = thing.split(';')
                    Ann.done_task(Ann.today[index], float(
                        things[0]), day0=CURRENTDAY)
                    ...
                    newT.start_time = time1.replace(
                        year=date.year, month=date.month, day=date.day)
                    newT.end_time = time2.replace(
                        year=date.year, month=date.month, day=date.day)
                    Cnn.add_Titems(newT)
                    if len(things) == 3:
...
def show_as_type():
    ...
    button1 = Button(TOP, text="Yes", command=lambda: show_type(
        Lb1.get(Lb1.curselection()[0])))
    ...
    for i in range(len(Ent)):
        ...
        if len(thing) != 0:
            ...
            time = datetime.datetime.strptime(
                StringVars[m1 + i].get(), "%H:%M")
            tit = Titem(thing)
            tit.start_time = time.replace(
                year=date.year, month=date.month, day=date.day)
...
def update_eff():
    ...
    for i in range(len(Cnn.Titems)):
        ...
        duration = (Cnn.Titems[i].end_time -
                    Cnn.Titems[i].start_time).seconds / 3600
...
def show_today_cal():
    ...
    for tit in Cnn.Titems:
        StringVars.append(StringVar())
        Labels.append(Label(TOP, textvariable=StringVars[i]))
        StringVars[i].set(tit.start_time.strftime("%H:%M") +
                          "-" + tit.end_time.strftime("%H:%M"))
        Labels[i].grid(row=i, column=0)
...
def init_menu(top):
    ...
    filemenu.add_command(
        label="Import from csv for calendar", command=donothing)
    ...
    filemenu.add_command(label="Export csv for calendar",
    ...
    taskmenu.add_command(
        label="Show with prority and urgent", command=donothing)
    taskmenu.add_command(label="Finished tasks", command=finish_task)
    taskmenu.add_command(label="Show all tasks", command=show_all)
    if ssss == 1:
        taskmenu.add_command(
            label="Switch to other tasklist", command=switch_t)
    ...
    calendarmenu.add_command(
        label="Return to yesterday", command=assign_yesterday)
    ...
    reportmenu.add_command(
        label="This week task tracker", command=report_tracker)
    reportmenu.add_command(
        label="Get this week file change", command=file_tracker)
