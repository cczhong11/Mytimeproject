...
class Report(object):
    ...
    def what_i_have_done(self, tasklist, ind, yom=0):
        ...
        dot = figure(title="what i have done", tools="", toolbar_location=None, x_range=[
                     0, max(self.yname)], y_range=self.xname)

        dot.segment(0, self.xname, self.yname, self.xname,
                    line_width=2, line_color="green", )
        dot.circle(self.yname, self.xname, size=15,
    ...
    def habit_tracker(self, tasklist, show_tasks, ind, yom=0):
        ...
        if yom == 0:
            ...
            for one in show_tasks:
                for i in range(7):
                    k = (nstart + datetime.timedelta(days=i * 1)
                         ).strftime("%Y-%m-%d")
                    days.append(k)
    ...
    def where_did_i_spend_time(self, cal, ind, yom=0):
        ...
        source = ColumnDataSource({'type': list(df['type']), 'detail_type': list(
            df['detail_type']), 'duration': list(df['duration'])})
        grouped = df.groupby(['type', 'detail_type']).sum()
        grouped.to_csv("report/report_time_week_" +
                       str(ind) + ".csv", encoding='utf-8')
        d = Donut(data=df, label=['type', 'detail_type'],
                  values='duration', text_font_size='6pt')
        d.plot_width = 800
        d.plot_height = 600
        return(d)
    ...
    def where_did_i_not_spend_time_m(self, cal, ind, yom=1):
        ...
        source = ColumnDataSource({'type': list(df['type']), 'detail_type': list(
            df['detail_type']), 'duration': list(df['duration'])})
        grouped = df.groupby(['type', 'detail_type']).sum()
        grouped.to_csv("report/report_not_time_week_" +
                       str(ind) + ".csv", encoding='utf-8')
        d = Donut(data=df, label=['type', 'detail_type'],
                  values='duration', text_font_size='6pt')
        d.plot_width = 800
    ...
    def efficient_time(self, cal, ind, yom=0):
        ...
        dot1 = figure(title="what i did not do", tools="", toolbar_location=None, x_range=[
                      0, max(yname1)], y_range=xname1)

        dot1.segment(0, xname1, yname1, xname1,
                     line_width=2, line_color="green", )
        dot1.circle(yname1, xname1, size=15, fill_color="orange",
        ...
        dot2 = figure(title="what i real do", tools="", toolbar_location=None, x_range=[
                      0, max(yname2)], y_range=xname2)

        dot2.segment(0, xname2, yname2, xname2,
                     line_width=2, line_color="green", )
        dot2.circle(yname2, xname2, size=15, fill_color="orange",
                    line_color="green", line_width=3, )
    ...
    def rescuetime_result(self, week, ind=0):
        ...
        if ind == 0:
            nstart = datetime.date(2018, 1, 1) + \
                datetime.timedelta(days=7 * (week - 1))
            result = get_all_from_rescuetime(
                [nstart + datetime.timedelta(days=i) for i in range(0 - nstart.weekday(), 7 - nstart.weekday())])
    ...
    def weekly(self, week):
        ...
        output_file("report/report_pie_" + str(week) +
                    ".html", title="weekly summary")
    ...
    def weekly_summary(self, week):
        ...
        output_file("report/report_tracker_" + str(week) +
                    ".html", title="habit tracker")
...
def get_time_delta(tup):
    ...
    a = datetime.datetime.strptime(
        tup[3], "%H:%M:%S") - datetime.datetime.strptime(tup[2], "%H:%M:%S")
